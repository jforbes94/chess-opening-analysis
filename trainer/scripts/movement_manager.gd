...
var pending_promotion_tile:  ColorRect = null
var pending_promotion_color: String    = ""
var last_moved_tile: ColorRect         = null
var piece_manager   = null

# player_color: "w" for white drills, "b" for black drills
...
const COLOR_HINT      = Color(0.2, 0.6, 1.0)

func _ready() -> void:
	var trainer    = get_parent()
	board_root     = trainer.get_node("BoardTiles")
	promotion_popup = trainer.get_node("UI/PromotionPopup")
...
func show_hint() -> void:
	if solution_index >= solution_moves.size():
		return
	var move     = solution_moves[solution_index]
	var to_name  = move.substr(2, 2)
	var to_tile  = board_root.get_node_or_null(to_name)
	if to_tile:
		to_tile.color = COLOR_HINT
		await get_tree().create_timer(1.0).timeout
		to_tile.color = _get_tile_color(to_tile)
...
func handle_tile_click(tile: ColorRect) -> void:
	...
	if selected_tile == null:
		...
		if _is_valid_move(moved_piece, old_pos, new_pos) and is_move_safe(moved_piece, old_pos, new_pos):
			...
			# Check solution in puzzle mode
			if puzzle_mode and GameStateManager.current_turn == player_color:
...
func end_turn() -> void:
	...
	# Auto-play if it's now the opponent's turn
	if GameStateManager.current_turn != player_color:
		call_deferred("_play_scripted_opponent_move")

func _play_scripted_opponent_move() -> void:
	await get_tree().create_timer(0.35).timeout
	if solution_index >= solution_moves.size():
		GameStateManager.current_turn = player_color
		emit_signal("puzzle_complete")
		return
...
func _apply_uci_move(move: String) -> void:
	...
	if castling_map.has(base):
		...
		var rf = board_root.get_node_or_null(game_state.indices_to_square_name(rook_from.x, rook_from.y))
		var rt = board_root.get_node_or_null(game_state.indices_to_square_name(rook_to.x,   rook_to.y))
		if rf and rt:
			move_piece(rf, rt, rook_piece)
...
func _flash_wrong(tile: ColorRect) -> void:
	tile.color = COLOR_WRONG
	await get_tree().create_timer(0.4).timeout
	tile.color = _get_tile_color(tile)
